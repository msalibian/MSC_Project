...
function bpsk_config_wrapper(out_dir, N, P, snrdB_vec, nX)
	...
	%L = 1000;
	%for l = 1:L
	%	[temp, sigpow_tmp] = bpsk_modulate(ceil(2*N/sps/dsss), fs, sps, fif, dsss, 1, 1, 1);
	...
	for s = 1:length(snrdB_vec)
		...
		for p = 1:P
            ...
            [xn_tmp, temp, nn_tmp] = bpsk_modulate(ceil(2*N/sps/dsss), fs, sps, fif, dsss, sd, norm_factor, gc);
